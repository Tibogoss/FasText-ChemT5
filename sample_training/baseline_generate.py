...
def main(baseline: str):
    # Load pretrained model and tokenizer
    device = "cuda" if torch.cuda.is_available() else "cpu"

    model_name = f"GT4SD/multitask-text-and-chemistry-t5-{baseline}-augm"
    ...
    output_file = f"captions_baseline_{baseline}.txt"
    with open(output_file, "w") as f:
        for caption in generated_captions:
            f.write("\t".join(caption) + "\n")

    print(f"Generated captions of {baseline} model saved to", output_file)
