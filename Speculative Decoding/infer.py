...
import numpy as np
import torch
from sampling.base_decoding_codec import autoregressive_generate
from sampling.speculative_decoding_codec import speculative_generate
...
class InferenceCLI:
    ...
    def __init__(self, device: str = "cuda"):
        ...
        self.chat = False # If using a chat instructed model, set to True
    ...
    def _load_models(self):
        # Target model
        target_model = "GT4SD/multitask-text-and-chemistry-t5-base-augm"  # "google/gemma-7b-it" "apple/OpenELM-3B"
        """ ckp1 = "./base_checkpoint.pt"
        drafter_model.load_state_dict(torch.load(ckp1)) """
        target_quantize = None  # QuantoConfig(weights="int8")
        
        # Drafter model
        drafter_model = "GT4SD/multitask-text-and-chemistry-t5-small-augm"  # "google/gemma-2b-it" "apple/OpenELM-270M"
        """ ckp2 = "./small_checkpoint.pt"
        drafter_model.load_state_dict(torch.load(ckp2)) """
        ...
        self.target = AutoModelForSeq2SeqLM.from_pretrained(
            target_model,
        ...
        ckp1 = "./base_checkpoint.pt"
        self.target.load_state_dict(torch.load(ckp1, map_location="cuda:1"))
        self.target.eval()
        ...
        self.drafter = AutoModelForSeq2SeqLM.from_pretrained(
            drafter_model,
        ...
        ckp2 = "./small_checkpoint.pt"
        self.drafter.load_state_dict(torch.load(ckp2, map_location="cuda:1"))
        self.drafter.eval()
